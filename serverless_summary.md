...
#### 소규모 전자상거래 웹사이트
* 소규모로 로딩 시간이 적당히 걸리더라도 감당할 수 있는 규모
----

#### 임시 웹사이트
* 짧은 이벤트 기간 동안 많은 사용자를 수용해야 하는 사이트
* 접속자 수에 따라 처리용량을 조절하고 빠른 개발을 제공하기에 서버리스가 적합하다.
----

#### 트리거 처리
* 레스트풀 서비ㅡ에 이미지를 보내는 모바일 애플리케이션
* N 스크린에 맞춰 적절한 이미지를 만드는 기능을 트리거라고 한다.
----

#### 챗봇
* 챗봇은 단순하며 특정 사례에 맞게 설계됐다.
* 제한된 시나리오의 서비스에 서버리스 백엔드가 유용한다.
----

#### IoT 백엔드
* 일련의 간단한 메시지 처리가 필요하며 이러한 처리에 서버리스가 적절하다.
----

#### 예정된 일정
* 전용시스템은 잠깐의 실행을 위해 시스템을 항상 켜두어야 한다.
* 서버리스는 사용 시간 만큼만 과금을 한다.
----

#### 빅데이터
* 데이터 덩어리를 서버리스의 동시처리기능에 로딩해서 사용 가능하다.
* 관리가 줄어들고 사용이 간편해진다.
* 다만 지속적인 데이터 사용은 비용이 높아질 수 있다.
----

#### 피해야할 것 들
* 다음의 기능을 가진 애플리케이션은 피해야 한다.
